...
#calculate gravitational force between all planets and sun
...
v_max = np.amax(v_rad)
noise = np.random.normal(0,.2*v_max,n)#Draw random samples from a normal (Gaussian) distribution.

plt.plot(v_rad+noise,label='v_rad')
plt.legend()
plt.show()
